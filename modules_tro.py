...
def write_image(xg, pred_label, gt_img, gt_label, tr_imgs, xg_swap, pred_label_swap, gt_label_swap, title, num_tr=2):
    """
    نسخه فارسی با PIL حرفه‌ای (arabic_reshaper + bidi)
    برای رندرینگ صحیح متن‌های فارسی با اتصال حروف و جهت RTL
    """
    ...
    try:
        ...
        USE_ADVANCED = False
        print("⚠️ برای نمایش بهینه، نصب کنید: pip install python-bidi arabic-reshaper")
    ...
    # تابع کمکی برای رسم متن فارسی
    def draw_persian_text(img_array, text, y_pos=55):
        """رسم متن فارسی با PIL و پشتیبانی کامل از RTL و اتصال حروف"""
        # تبدیل به تصویر رنگی برای PIL
        ...
        # اصلاح شکل و جهت متن
        if USE_ADVANCED and text.strip():
            reshaped_text = reshape(text)
            display_text = get_display(reshaped_text)
        else:
            # Fallback: معکوس ساده
            display_text = text[::-1] if text.strip() else text
        
        # رسم متن
        draw.text((5, y_pos-25), display_text, fill=(255, 255, 255), font=font)
    ...
    for i in range(batch_size):
        ...
        gt_text_img = draw_persian_text(gt_text_img, gt_text)
        gt_text_img_swap = draw_persian_text(gt_text_img_swap, gt_text_swap)
        pred_text_img = draw_persian_text(pred_text_img, pred_text)
        pred_text_img_swap = draw_persian_text(pred_text_img_swap, pred_text_swap)
    ...
    # ترکیب تصاویر تمام batch به صورت افقی
...
class DisModel(nn.Module):
    ...
    def forward(self, x):
        feat = self.cnn_f(x)
        out = self.cnn_c(feat)
        return out.squeeze(-1).squeeze(-1)  # b,1024
