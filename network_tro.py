...
class ConTranModel(nn.Module):
    ...
    def forward(self, train_data_list, epoch, mode, cer_func=None):
        ...
        if mode == 'rec_update':
            ...
            '''l1 loss'''
            if self.oov or w_l1 == 0:
                l_l1 = torch.tensor(0.).to(gpu)
            else:
                l_l1 = recon_criterion(xg, img_xt)
                # # 🔥 دیباگ: چاپ آمار
                # if self.iter_num % 50 == 1:
                #     print(f"[DEBUG] xg: min={xg.min():.3f}, max={xg.max():.3f}, mean={xg.mean():.3f}")
                #     print(f"[DEBUG] img_xt: min={img_xt.min():.3f}, max={img_xt.max():.3f}, mean={img_xt.mean():.3f}")
                #     print(f"[DEBUG] l_l1 (raw)={l_l1.item():.6f}, weighted={w_l1 * l_l1.item():.2f}")
